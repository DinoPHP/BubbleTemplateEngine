 ...
 * @group functional
 */
class Bubble_Test_Functional_HigherOrderSectionsTest extends Bubble_Test_FunctionalTestCase
{
    private $bubble;
    ...
    {
        $foo = new Bubble_Test_Functional_Foo();
        $foo->doublewrap = array($foo, 'wrapWithBoth');

        $bar = new Bubble_Test_Functional_Foo();
    ...
    {
        $tpl = $this->bubble->loadTemplate('{{#trim}}    {{name}}    {{/trim}}');
    ...
    {
        $tpl = $this->bubble->loadTemplate('{{#title}}{{title}} {{/title}}{{name}}');

        $frank = new Bubble_Test_Functional_Monster();
        $frank->title = 'Dr.';
        $frank->name  = 'Frankenstein';
        $this->assertEquals('Dr. Frankenstein', $tpl->render($frank));

        $dracula = new Bubble_Test_Functional_Monster();
        $dracula->title = 'Count';
    ...
    {
        $bubble = $this->getMock('Bubble_Engine', array('loadLambda'));
        $bubble->expects($this->never())
            ->method('loadLambda');

        $tpl = $bubble->loadTemplate('{{#wrap}}NAME{{/wrap}}');
    ...
    {
        $bubble = $this->getMock('Bubble_Engine', array('loadLambda'));
        $bubble->expects($this->once())
            ->method('loadLambda')
            ->will($this->returnValue($bubble->loadTemplate('<em>{{ name }}</em>')));
...
class Bubble_Test_Functional_Foo
{
    public $name = 'Justin';
...
class Bubble_Test_Functional_Monster
{
    public $title;
    public $name;
}
