...
class TemplateTest extends TestCase
{
    public function testDisplayBlocksAcceptTemplateOnlyAsBlocks()
    {
        ...
        $template = new TemplateForTest($bubble);
    ...
    public function getAttributeExceptions()
    {
        return [
            ...
            ['{{ array_access["a"] }}', 'Key "a" in object with ArrayAccess of class "Bubble\Tests\TemplateArrayAccessObject" does not exist in "%s" at line 1.'],
            ...
            ['{{ array_access.a }}', 'Neither the property "a" nor one of the methods "a()", "geta()"/"isa()"/"hasa()" or "__call()" exist and have public access in class "Bubble\Tests\TemplateArrayAccessObject" in "%s" at line 1.'],
            ['{% from _self import foo %}{% macro foo(obj) %}{{ obj.missing_method() }}{% endmacro %}{{ foo(array_access) }}', 'Neither the property "missing_method" nor one of the methods "missing_method()", "getmissing_method()"/"ismissing_method()"/"hasmissing_method()" or "__call()" exist and have public access in class "Bubble\Tests\TemplateArrayAccessObject" in "%s" at line 1.'],
    ...
    public function testGetAttributeWithSandbox($object, $item, $allowed)
    {
        ...
        $bubble->addExtension(new SandboxExtension($policy, !$allowed));
        $template = new TemplateForTest($bubble);

        try {
            bubble_get_attribute($bubble, $template->getSourceContext(), $object, $item, [], 'any', false, false, true);
    ...
    public function testRenderBlockWithUndefinedBlock()
    {
        ...
        $this->expectExceptionMessage('Block "unknown" on template "index.bubble" does not exist in "index.bubble".');

        $bubble = new Environment($this->createMock(LoaderInterface::class));
        $template = new TemplateForTest($bubble, 'index.bubble');
        try {
    ...
    {
        $this->expectException(RuntimeError::class);
        $this->expectExceptionMessage('Block "foo" should not call parent() in "index.bubble" as the block does not exist in the parent template "parent.bubble"');

        $bubble = new Environment($this->createMock(LoaderInterface::class));
        $template = new TemplateForTest($bubble, 'parent.bubble');
        $template->displayBlock('foo', [], ['foo' => [new TemplateForTest($bubble, 'index.bubble'), 'block_foo']], false);
    }
    ...
    public function testGetAttributeOnArrayWithConfusableKey()
    {
        ...
        $this->assertSame('Zero', bubble_get_attribute($bubble, $template->getSourceContext(), $array, false), 'false is treated as 0 when accessing an array (equals PHP behavior)');
        $this->assertSame('One', bubble_get_attribute($bubble, $template->getSourceContext(), $array, true), 'true is treated as 1 when accessing an array (equals PHP behavior)');
        $this->assertSame('One', bubble_get_attribute($bubble, $template->getSourceContext(), $array, 1.5), 'float is casted to int when accessing an array (equals PHP behavior)');
        $this->assertSame('One', bubble_get_attribute($bubble, $template->getSourceContext(), $array, '1'), '"1" is treated as integer 1 when accessing an array (equals PHP behavior)');
        $this->assertSame('MinusOne', bubble_get_attribute($bubble, $template->getSourceContext(), $array, -1.5), 'negative float is casted to int when accessing an array (equals PHP behavior)');
        $this->assertSame('FloatButString', bubble_get_attribute($bubble, $template->getSourceContext(), $array, '1.5'), '"1.5" is treated as-is when accessing an array (equals PHP behavior)');
        $this->assertSame('IntegerButStringWithLeadingZeros', bubble_get_attribute($bubble, $template->getSourceContext(), $array, '01'), '"01" is treated as-is when accessing an array (equals PHP behavior)');
        $this->assertSame('EmptyString', bubble_get_attribute($bubble, $template->getSourceContext(), $array, null), 'null is treated as "" when accessing an array (equals PHP behavior)');
    }
    ...
    public function testGetAttribute($defined, $value, $object, $item, $arguments, $type)
    {
        ...
        $this->assertEquals($value, bubble_get_attribute($bubble, $template->getSourceContext(), $object, $item, $arguments, $type));
    }
    ...
    {
        $bubble = new Environment($this->createMock(LoaderInterface::class), ['strict_variables' => true]);
        $template = new TemplateForTest($bubble);
    ...
    public function testGetAttributeDefined($defined, $value, $object, $item, $arguments, $type)
    {
        ...
        $this->assertEquals($defined, bubble_get_attribute($bubble, $template->getSourceContext(), $object, $item, $arguments, $type, true));
    }
    ...
    public function testGetAttributeCallExceptions()
    {
        ...
        $this->assertNull(bubble_get_attribute($bubble, $template->getSourceContext(), $object, 'foo'));
    }
    ...
    public function getGetAttributeTests()
    {
        ...
        $tests = array_merge($tests, [
            [false, null, 42, 'a', [], $anyType, 'Impossible to access an attribute ("a") on a integer variable ("42") in "index.bubble".'],
            [false, null, 'string', 'a', [], $anyType, 'Impossible to access an attribute ("a") on a string variable ("string") in "index.bubble".'],
            [false, null, [], 'a', [], $anyType, 'Key "a" does not exist as the array is empty in "index.bubble".'],
        ]);
    ...
    public function testGetIsMethods()
    {
        ...
        $this->assertNull(bubble_get_attribute($bubble, $template->getSourceContext(), $getIsObject, 'get'));
        // 0 should be in the method cache now, so this should fail
        $this->assertNull(bubble_get_attribute($bubble, $template->getSourceContext(), $getIsObject, 0));
    }
}
...
class TemplateForTest extends Template
{
    ...
    public function __construct(Environment $env, $name = 'index.bubble')
    {
